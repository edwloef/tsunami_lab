...
tsunami_lab::t_real
tsunami_lab::setups::SubCritical1d::getHeight(t_real i_x, t_real i_y) const {
    return -getBathymetry(i_x, i_y);
}

tsunami_lab::t_real
tsunami_lab::setups::SubCritical1d::getMomentumX(t_real, t_real) const {
    return 4.42;
}
...
tsunami_lab::setups::SubCritical1d::getBathymetry(t_real i_x, t_real) const {
    return i_x > 8 && i_x < 12 ? -1.8 - 0.05 * std::powf(i_x - 10, 2) : -2;
}